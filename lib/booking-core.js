(function attachBookingCore(root, factory) {
  ...
  const MAX_WINDOW_END_SECONDS = 5 * 60;
  ...
  function courtNamesMatch(left, right) {
    const identity = (value) => normalizeCourtName(value)
  ...
  function findAnyConsecutivePair(slots, courtOrder = []) {
    const starts = [...new Set(slots.map((slot) => parseStartMinutes(slot.slotText))
      .filter((start) => start !== null))].sort((left, right) => left - right);
    const normalizedOrder = courtOrder.map(normalizeCourtName);
    const remainingCourts = [...new Set(slots.map((slot) => normalizeCourtName(slot.court)))]
      .filter((court) => !normalizedOrder.includes(court));

    for (const court of [...normalizedOrder, ...remainingCourts]) {
      const pair = findConsecutiveSlotPair(
        slots.filter((slot) => normalizeCourtName(slot.court) === court),
        starts
      );
      if (pair) return pair;
    }
    return findSameAreaCourtPair(slots, starts, courtOrder);
  }

  function rankExactHourFallback(matches, bookedMatch, missingSlotText, courtOrder = []) {
    const bookedDetails = courtAreaDetails(bookedMatch?.court);
    if (!bookedDetails) return [];
    const courtRank = new Map(courtOrder.map((court, index) => [normalizeCourtName(court), index]));
    return matches
      .filter((match) => normalizeSpace(match.slotText) === normalizeSpace(missingSlotText))
      .filter((match) => courtAreaDetails(match.court)?.area === bookedDetails.area)
      .sort((left, right) => {
        const leftDetails = courtAreaDetails(left.court);
        const rightDetails = courtAreaDetails(right.court);
        const sameCourtDifference = Number(!courtNamesMatch(left.court, bookedMatch.court)) -
          Number(!courtNamesMatch(right.court, bookedMatch.court));
        if (sameCourtDifference) return sameCourtDifference;
        const distanceDifference = Math.abs(leftDetails.number - bookedDetails.number) -
          Math.abs(rightDetails.number - bookedDetails.number);
        if (distanceDifference) return distanceDifference;
        return (courtRank.get(normalizeCourtName(left.court)) ?? 999) -
          (courtRank.get(normalizeCourtName(right.court)) ?? 999);
      });
  }
  ...
  function addDaysToDateKey(value, days = 1) {
    if (!isValidDateKey(value) || !Number.isInteger(days)) return null;
    const [year, month, day] = value.split("-").map(Number);
    return localDateKey(new Date(year, month - 1, day + days, 12, 0, 0, 0));
  }
  ...
  function validateSettings(input) {
    ...
    if (pollSeconds !== MIN_POLL_SECONDS) {
      errors.push(`Availability checking is fixed at every ${MIN_POLL_SECONDS} second.`);
    }
    ...
    if (windowStartSeconds !== 0 || windowEndSeconds !== MAX_WINDOW_END_SECONDS) {
      errors.push("Monitoring is fixed at 12:00 AM–12:05 AM.");
    }
  ...
  return {
    ...
    findAnyConsecutivePair,
    findSameAreaCourtPair,
    rankExactHourFallback,
